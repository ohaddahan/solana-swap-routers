...
async fn test_titan_quote_and_swap_direct_only() {
    ...
    let quote = aggregator
        ...
        .await
        .expect("titan direct-only quote should succeed");
    ...
    print_quote("titan::quote_and_swap_direct", &quote);

    let swap_result = aggregator
        .swap(&quote, &pubkey, &rpc_client)
        .await
        .expect("titan direct-only swap should succeed");

    finalize_swap(
        "titan::quote_and_swap_direct",
        swap_result,
        &env.keypair,
        &rpc_client,
        env.send_tx,
