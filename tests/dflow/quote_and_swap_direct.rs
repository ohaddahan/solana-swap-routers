...
async fn test_dflow_quote_and_swap_direct_only() {
    ...
    let quote = aggregator
        ...
        .await
        .expect("dflow direct-only quote should succeed");
    ...
    print_quote("dflow::quote_and_swap_direct", &quote);

    let swap_result = aggregator
        .swap(&quote, &pubkey, &rpc_client)
        .await
        .expect("dflow direct-only swap should succeed");

    finalize_swap(
        "dflow::quote_and_swap_direct",
        swap_result,
        &env.keypair,
        &rpc_client,
        env.send_tx,
