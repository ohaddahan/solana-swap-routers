...
async fn test_jupiter_quote_and_swap_direct_only() {
    let env = load_test_env();
    let aggregator = SwapAggregator::new(build_swap_config(&env, None));
    let request = build_quote_request(&env, Some(true));
    ...
    let quote = aggregator
        ...
        .await
        .expect("jupiter direct-only quote should succeed");
    ...
    print_quote("jupiter::quote_and_swap_direct", &quote);

    let swap_result = aggregator
        .swap(&quote, &pubkey, &rpc_client)
        .await
        .expect("jupiter direct-only swap should succeed");

    finalize_swap(
        "jupiter::quote_and_swap_direct",
        swap_result,
        &env.keypair,
        &rpc_client,
        env.send_tx,
